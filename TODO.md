...
### 🗄 Raw storage: DuckDB (MongoDB removed, ECO-158/159)
DuckDB (`ingestion/storage/duckdb_store.py`) is the sole raw store —
every trigger script and `ingestion/api.py`'s `/ingestion/historical` /
`/ingestion/retry-missing` endpoints write to it directly; `RawSyncer`
reads from it to populate Postgres `raw.*` for dbt. No MongoDB code
remains in the ingestion pipeline (`ingestion/storage/mongo.py` and its
tests deleted, `motor`/`pymongo` dropped).
- [✓] [ECO-158] DuckDB historical store (`write_historical()`/
  `read_historical()`/`read_historical_since()`/`latest_fetched_at()`/
  `count_by_day()`).
- [✓] [ECO-159] Full MongoDB removal across ingestion/warehouse/scripts.
  Full suite green (680 tests).
- [✓] [ECO-162] **DuckDB backup/restore safeguard.** Removing MongoDB
  means there's no longer a remote redundant copy of raw history — the
  local `.duckdb` file is now a single point of failure (disk loss,
  accidental `rm`, corruption). Added `scripts/backup_duckdb.py`
  (read-only `EXPORT DATABASE` snapshot to timestamped Parquet, safe to
  run concurrently with the live writer, retention via `--keep`) and
  `scripts/restore_duckdb.py` (`IMPORT DATABASE` from a snapshot into a
  fresh file, refuses to clobber an existing one without `--force`).
  `make backup-duckdb` / `make restore-duckdb SNAPSHOT=...`. Covered by
  `tests/test_backup_duckdb.py` (real write → backup → restore
  roundtrip, retention pruning). Still manual/cron, not wired into a
  schedule yet.
- [ ] [ECO-160] **Update `CLAUDE.md`'s remaining Mongo-era framing.**
  Lower priority (read by an AI agent, not user-facing), but worth a pass.
- [ ] [ECO-161] **`.github/workflows/ingest.yml` doesn't persist DuckDB
  data across runs.** The file lives on the ephemeral GitHub Actions
  runner and is discarded when the job ends. `scripts/backup_duckdb.py`
  (ECO-162) produces the snapshot format this would restore-before/
  upload-after via `actions/cache` or an S3 sync — just not wired up yet.
