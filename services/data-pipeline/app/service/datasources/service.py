...
2. Admin-editable config/overrides (`enabled`, `cron`, `timezone`,
   ...
   `meta.data_sources`. NULL columns there mean "no override, use the
   catalog default"; `metadata` merges on top of the catalog's metadata
...
`datasources:one:v1:{id}` (30s, single). `PATCH` clears both — the single
key for the id it just changed, and every list key (list results can't be
selectively invalidated since a changed source might now match/not-match
an arbitrary filter combination baked into some cached `query_hash`).
"""
...
# Back-compat alias — this was the name before the single-item endpoint
# existed; kept so nothing importing it breaks.
CACHE_TTL_SECONDS = LIST_CACHE_TTL_SECONDS
CACHE_KEY_PREFIX = LIST_CACHE_PREFIX
_STATS_WINDOW = timedelta(days=7)

# API_SPECEFICATIONS.md's literal regex (`^(\*|[0-9,\-\/]+)( [0-9,\-\/]+){4}$`)
# can't actually match `*/N` step syntax — its first-field alternative is
# either a bare `*` or digits/commas/hyphens/slashes with no `*` mixed in,
# so `*/10` (the spec's own PATCH example, and every cron in our catalog:
# `*/5`, `*/15`, `*/30`) fails it. Widened here to allow `*` within the
# character class so step syntax validates, while keeping the same
# "5 space-separated fields" shape check.
