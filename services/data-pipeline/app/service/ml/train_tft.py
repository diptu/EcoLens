...
def _predict_tft(
    model: DemandTFT, loader: DataLoader
...
def _compute_val_loss_tft(
    model: DemandTFT, loader: DataLoader, quantile_weight: float
) -> float:
...
def train_tft_model(
    ...
    *,
    ...
    for epoch in range(config.epochs):
        ...
        y_val, _, p50_val, _ = _predict_tft(model, val_loader)
        ...
        val_loss = _compute_val_loss_tft(model, val_loader, config.quantile_weight)
    ...
    y_cal, p10_cal, _, p90_cal = _predict_tft(model, cal_loader)
    ...
    if test_loader is not None:
        y_test, p10_test, p50_test, p90_test = _predict_tft(model, test_loader)
