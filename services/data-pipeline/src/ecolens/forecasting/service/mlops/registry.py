...
from ecolens.forecasting.schema.features import FeatureScaler

log = get_logger(__name__)


class ForecastingModel(Protocol):
    """Structural type both `DemandLSTM` and `DemandTFT` satisfy without
    any shared base class -- everything `log_model_artifacts`/
    `load_by_alias` need from an architecture-agnostic forecasting model.
    Deliberately *not* typed as `nn.Module` directly: that stub routes
    unknown-attribute access (like `architecture_dict`) through
    `nn.Module.__getattr__`, which mypy types as returning
    `Tensor | Module` -- this Protocol sidesteps that so
    `model.architecture_dict()` type-checks as the real return type.
    """
...
ModelT = TypeVar("ModelT", bound=ForecastingModel)


def log_model_artifacts(
    model: ForecastingModel, *, optimizer_state: dict[str, Any] | None = None
) -> None:
    ...
    Type-hinted as `ForecastingModel` (not `DemandLSTM`) since this is
    architecture-agnostic in practice -- `training/train_tft.py` calls
    this with a `DemandTFT` exactly the same way `training/train.py`
    calls it with a `DemandLSTM`. The only requirement is that `model`
    implements `architecture_dict()` (both do).
...
class ModelRegistry:
    ...
    @overload
    def load_by_alias(
        self, alias: str, *, device: str = "cpu", model_type: type[ModelT]
    ) -> ModelT: ...

    def load_by_alias(
        self, alias: str, *, device: str = "cpu", model_type: type[Any] = DemandLSTM
    ) -> Any:
        ...
        `model_type` defaults to `DemandLSTM` for backward compatibility
        with every existing caller -- the two `@overload`s above are what
        let an unchanged call site keep getting `DemandLSTM` back
        precisely (not a widened `ForecastingModel`/`Any`), since mypy
        doesn't infer a generic default from an *omitted* argument on its
        own. A TFT caller passes `model_type=DemandTFT` to load from
        `self.model_name`'s own registry (which, for TFT, is a
        `mlflow_registered_model_name_tft`-scoped `ModelRegistry`, never
        the LSTM's) and gets `DemandTFT` back precisely in exchange.
        """
        import torch
        ...
        if not isinstance(model, model_type):
            raise TypeError(
                f"models:/{self.model_name}@{alias} did not load as a "
                f"{model_type.__name__} (got {type(model).__name__}) -- registry "
                "may hold a model from an incompatible training run"
            )
        return model
