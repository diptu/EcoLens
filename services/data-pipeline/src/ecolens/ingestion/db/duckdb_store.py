...
live fetcher (`service/*/engine.py`) and every historical backfill
(`HistoricalFetcher`, `scripts/backfill_bom_historical.py`,
`ingestion/api/routes.py`'s `/ingestion/historical`) writes here via
`write_historical()`. `ingestion/repository/raw_sync.py`'s `RawSyncer`
reads from these tables to populate Postgres `raw.*`, which dbt builds
from. Every successful `write_historical()` also fires a best-effort
RabbitMQ event (`ecolens.shared.events.rabbitmq.publish_data_ingested`)
that `ecolens.warehouse.service.event_consumer` listens for to trigger
an incremental warehouse run -- push-based instead of the old
every-30-min warehouse cron.
...
def write_historical(
    ...
    *,
    ...
    Root TODO.md's "Anomaly Detection" section: also stamps
    `anomaly_score`/`anomaly_flags`/`anomaly_explanation` onto every doc
    (`ingestion.service.anomaly.scorer.score_batch`, called here rather
    than from each of the 5 `service/*/engine.py` fetchers -- see that
    module's own docstring for why this one chokepoint is the right
    place). Purely additive, never drops or nulls a record for scoring
    high. Requires the 5 DuckDB tables to already carry these 3 columns
    -- run `scripts/migrate_add_anomaly_columns.py` once before this
    ships against an existing store, or `_upsert`'s `INSERT ... SELECT *
    FROM df` below fails with a column-count mismatch.
    """
    if not docs:
        return 0
    ...
    # Local import: `ingestion.service.anomaly.baseline` imports this
    # module (`duckdb_store`) to read recent history for the rolling
    # baseline -- a module-level import here would be circular
    # (duckdb_store -> scorer -> baseline -> duckdb_store). Same
    # local-import-to-break-a-cycle precedent `raw_sync.py`'s own
    # docstring already documents for its `warehouse.core.runner_settings`
    # import.
    ...
    publish_data_ingested(source, run_id=run_id, rows=len(docs))
    return len(docs)
