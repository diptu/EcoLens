...
`ecolens.ingestion.db.duckdb_store`) so "sync" here is a column
projection + upsert, not a real reshape.

The `raw.*` table/column contract (including each source's unique key)
must stay in lockstep with:
  - `ecolens.ingestion.core.settings.IngestionSettings` (table names,
...
`ecolens.warehouse.core.runner_settings` would pull in `warehouse.runner`'s
...
from ecolens.ingestion.core.circuit_breaker import retry_with_backoff
