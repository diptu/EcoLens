...
=====
    ...
    python -m ecolens.warehouse.core.runner_entrypoint --full

    # Incremental (default; normally triggered by event_consumer, not cron)
    python -m ecolens.warehouse.core.runner_entrypoint --incremental

    # Validate only (no dbt run; just check current state)
    python -m ecolens.warehouse.core.runner_entrypoint --validate-only

    # Run a specific dbt tag
    python -m ecolens.warehouse.core.runner_entrypoint --select tag:ml_features
"""
...
from .runner_settings import get_warehouse_runner_settings
...
def _show_example() -> int:
    ...
    print("=" * 70)
    print()
    print("$ python -m ecolens.warehouse.core.runner_entrypoint --incremental")
    print()
    print(
...
def parse_args() -> argparse.Namespace:
    ...
    mode.add_argument(
        ...
        action="store_true",
        default=True,
        help="incremental run (default; normally triggered by event_consumer, not cron)",
    )
    mode.add_argument(
        "--full",
        action="store_true",
