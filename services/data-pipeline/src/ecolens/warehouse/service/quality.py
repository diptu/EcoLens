...
#
# The 3 AEMO-derived facts use a 30h threshold, not a tight one: NEMWeb
# only publishes each AEST calendar day's dispatch file ~4am the
# *following* day (see AEMONEMFetcher.fetch_for_date's docstring), so
# under the current (whole-day-batch) ingestion design the latest row
# in these tables is *always* somewhere between ~4.5h and ~28.5h old,
# cycling with AEMO's own publish rhythm -- a 45min threshold would
# violate on every single incremental run, not just when something is
# actually broken.
FRESHNESS_CHECKS: list[tuple[str, str, timedelta]] = [
    ("fact_demand_30min", "ts_30", timedelta(hours=30)),
    ("fact_generation_30min", "ts_30", timedelta(hours=30)),
    ("ml_features_demand_v1", "ts_30", timedelta(hours=30)),
