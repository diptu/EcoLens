...
class FakeMlflowClient:
    ...
    async def get(self, url):
        return FakeMlflowResponse(self._status_code)
...
def test_readyz_returns_200_when_all_components_healthy(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.api.v1.health.routes.httpx.AsyncClient",
        lambda **kwargs: FakeMlflowClient(200),
    )
...
def test_readyz_returns_503_when_a_component_is_unhealthy(client, monkeypatch):
...
def test_readyz_marks_mlflow_unhealthy_on_non_200(client, monkeypatch):
    app.dependency_overrides[get_db] = lambda: OkSession()
    app.dependency_overrides[get_redis_client] = lambda: OkRedis()
    monkeypatch.setattr(
        "app.api.v1.health.routes.httpx.AsyncClient",
        lambda **kwargs: FakeMlflowClient(403),
    )

    response = client.get("/v1/readyz")

    assert response.status_code == 503
    mlflow = next(c for c in response.json()["components"] if c["name"] == "mlflow")
    assert mlflow["healthy"] is False
