...
class TestAuth:
    def test_missing_token_returns_401_envelope(self, client):
    ...
    def test_wrong_role_returns_403(self, client):
        response = client.get("/v1/data-sources", headers=_auth(role="viewer"))

        assert response.status_code == 403
        assert response.json()["error"]["code"] == "forbidden"

    def test_admin_role_is_allowed(self, client):
        response = client.get("/v1/data-sources", headers=_auth(role="admin"))

        assert response.status_code == 200

    def test_analyst_role_is_allowed(self, client):
        response = client.get("/v1/data-sources", headers=_auth(role="analyst"))
