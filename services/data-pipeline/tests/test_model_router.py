...
def fake_training_trigger_deps(monkeypatch):
    ...
    (anomaly count) and a real RabbitMQ publish -- fake both, same
    pattern `test_flows.py`'s `TestPublishTrainingTrigger` already uses
    for the exact same function (`.fn` still shares this module's
    namespace, so patching `flows.get_session`/`flows.
    publish_training_trigger_event` here affects the route's call
    through `trigger_training` too)."""
    published: list[dict] = []

    async def fake_publish(payload):
        published.append(payload)

    monkeypatch.setattr(flows, "get_session", lambda: _FakeSessionCtx(_FakeSession()))
...
def test_train_endpoint_requires_no_auth(client):
    # Deliberately open, same reasoning as /v1/data-sources/{id}/run --
    # this asserts the *absence* of a role gate, not a specific role.
    response = client.post("/v1/model/train", json={})
