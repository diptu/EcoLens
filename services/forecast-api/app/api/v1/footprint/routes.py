...
`load_intensity_over_period`, README's `live_mix_weighted` method)."""
...
from app.service.ml.data import load_intensity_over_period
...
async def compute_footprint(
    ...
    intensity_kgco2e_per_mwh = float(row["total_emissions_kgco2e"]) / float(
        row["total_generation_mwh"]
    )
