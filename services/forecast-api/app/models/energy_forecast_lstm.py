"""`EnergyForecastLSTM` — **intentionally duplicated, byte-for-byte, from
`data-pipeline`'s `app.models.energy_forecast_lstm`**, same reason
`app/models/ml.py`'s own docstring documents for `DemandLSTM`:
`data-pipeline` trains and persists a plain `state_dict` + architecture
hyperparams (not a pickled class reference), so this service reconstructs
the class from its own copy and loads weights in — no runtime dependency
on `data-pipeline`'s package. **This means the two copies must be kept in
sync by hand.**

The multi-task demand + generation-mix forecast model (`overview.md`'s
"forecasts future energy demand with detailed source breakdowns", ported
from `services/forecast-api/notebooks/lstm.ipynb`, which had never been
executed — no training/serving code existed anywhere for this
architecture before this module).

Differs from `DemandLSTM` (`app/models/ml.py`) in two structural ways,
both deliberate, not oversights:

  1. **Multi-horizon by direct decoding, not attention pooling.**
     `DemandLSTM` pools the lookback window into one context vector via
     `AttentionPool`, then projects that single vector straight to all
     `horizon` outputs with linear heads. This model instead runs a
     second LSTM (`decoder`) over `horizon` steps, each step seeded with
     the same encoder context plus a learned per-step `horizon_embedding`
     -- still not autoregressive (no step's prediction feeds the next),
     but gives the generation head room to vary its per-source *mix*
     across the horizon rather than applying one fixed transformation to
     every step.
  2. **Quantiles stacked on a trailing dim (`[..., 3]`), not three
     separate dataclass fields.** `DemandForecast` has independent
     `p10`/`p50`/`p90` tensors -- clean for a single `(batch, horizon)`
     target. Generation's natural shape is `(batch, horizon, sources)`,
     and three separate same-shaped fields per quantile would be
     equally verbose while losing the ability to do one vectorised
     pinball-loss call across all three at once (`losses.
     energy_forecast_loss` relies on this). `EnergyForecast` keeps the
     notebook's `[..., 3]` convention for exactly this reason -- see its
     own docstring for the index constants.

Both heads guarantee `P10 <= P50 <= P90` by construction (`softplus`
non-negative deltas stacked P10 -> P50 -> P90), the same
structural-non-crossing trick `DemandLSTM`'s spread heads already use,
just generalised to an arbitrary trailing `sources` dimension for
generation.
"""
