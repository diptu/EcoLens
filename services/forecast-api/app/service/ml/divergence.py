...
from app.core.metrics import forecast_drift_score
...
def check_drift(
    ...
    *,
    ...
    with tracer.start_as_current_span(
        "forecast_api.check_drift", attributes={"model_name": model_name}
    ) as span:
        anchor_run_id = find_last_full_retrain_run_id(model_name)
        if anchor_run_id is None:
            span.set_attribute("anchor_found", False)
            return None
        span.set_attribute("anchor_found", True)
        span.set_attribute("anchor_run_id", anchor_run_id)
        ...
        span.set_attribute("relative_l2_drift", drift)
        span.set_attribute("exceeded_threshold", exceeded)
        forecast_drift_score.labels(model_name=model_name).set(drift)
        return DriftReport(
