...
As of writing, `data-pipeline`'s `forecasting/` tree (including the registry client) is still an empty-stub scaffold (root `TODO.md` ECO-108–119) — but **`main` already has a baseline (non-LSTM) serving layer**: a seasonal-naive forecaster reading `data-pipeline`'s `ml_features_demand_v1` mart, plus the FastAPI app/Postgres pool/Redis cache/health check around it (see `TODO.md`'s "Baseline Serving Layer" note, ECO-F00). Everything below this point is forward-looking design for the *real-model* phase, layered on top of that baseline — not a description of what `main` currently runs.
...
*   **Feature window:** Maintain a `collections.deque` buffer on the CPU to sustain the sliding window required for LSTM time-series input (ECO-F05) — this reconstructs each request's `model_lookback`-length window from already-ingested features instead of re-querying Postgres per request.
...
## 8. Summary Checklist
- [x] Baseline serving layer live on `main` (FastAPI app, Settings, Postgres pool, Redis cache, `/health`, seasonal-naive `/v1/forecast/{region}`) — ECO-F00.
- [ ] Save model as `state_dict` (`data-pipeline`, ECO-112/ECO-115).
- [ ] Extend this service's `Settings` with model-loader config (ECO-F02).
...
- [ ] Swap the baseline forecaster for real model output with conformal P10/P50/P90 bands, same response contract (ECO-F06).
