...
Both endpoints return `202 Queued` immediately and hand the actual work
off to a FastAPI `BackgroundTasks` callback. There's no separate task
queue/worker in this service beyond that — see `RunTriggerResponse`/
`BackfillTriggerResponse`'s docstrings for what that does and doesn't
...
Scoped down from data-pipeline's identical module: `run_backfill_in_
background` here never triggers a `dbt build` afterward — that's
warehouse's job, not ingestion's (`pyproject.toml`'s own description).
"""
...
# Only these 3 chunk values are documented/accepted; execution is always
# day-granularity regardless (see BackfillTriggerResponse's docstring)
# so a generic ISO-8601-duration parser would be more general than
...
async def run_in_background(registry_key: str, force: bool, triggered_by: str) -> None:
    ...
    isn't threaded through here — the warehouse's `(pk) ON CONFLICT DO
    NOTHING` load already makes re-fetched rows a no-op, which is the
