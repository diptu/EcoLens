...
— read-only, higher-resolution views on top of the same `meta.
_ingest_log`/circuit-breaker data `app.service.datasources.service`
already summarises for the list/single endpoints. Ported from
data-pipeline's identical module.
"""
...
def _classify_error(message: str) -> str | None:
    """Best-effort keyword match into the fixed `errors_by_code_24h`
    vocabulary. `error_message` is a free-text exception string
    (`str(exc)[:500]`, `_common._log_run_finish`), not a structured
    error code, so this is a heuristic, not an exact classification —
    messages that don't match any keyword aren't counted under any code
    (undercounting is the failure mode, not silently mis-bucketing)."""
