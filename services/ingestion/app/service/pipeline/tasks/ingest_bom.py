...
Ported from `data-pipeline`'s identical module (`services/ingestion/
TODO.md` Phase 1, "Migrate Ingest Tasks"), with one real behavior
change carried along explicitly (not a silent drift): `_try_live_api`
now sends a `User-Agent` header. This service's own `notebooks/
ingestion.ipynb` found bom.gov.au returning a real `403 Forbidden`
without one -- `data-pipeline`'s version of this file never set one at
all. Everything else is verbatim.
"""
...
# bom.gov.au returns 403 Forbidden without a browser-like User-Agent --
# confirmed live from notebooks/ingestion.ipynb (2026-08-05), not
# assumed. AEMO's own archive fetch (`ingest_aemo_nem.py`'s
# `_fetch_historical_range`) already sets the same header for the same
# reason.
_BOM_USER_AGENT = "Mozilla/5.0"
