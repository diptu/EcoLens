#!/usr/bin/env python3
"""Backfill AEMO WEM (SWIS) demand + price for a real date range and
write it into its own **separate** local DuckDB file -- not the shared
...
Reuses `ingest_aemo_wem._fetch_historical_range` unchanged -- the same
real fetch `pipeline.backfill`/the CLI's `ingest aemo-wem --start/--end`
already use (real AEMO WA `operationalDemandWithdrawal`/
`referenceTradingPrice` archives, `ingest_aemo_wem.py`'s own docstring
has the two endpoints). Mirrors `backfill_nem_to_duckdb.py` exactly, one
source swapped for the other.

Run from `services/ingestion/`:

    uv run python scripts/backfill_wem_to_duckdb.py \\
        --from 2025-07-01 --to 2025-07-31 \\
        --output data/historical_wem.duckdb
"""
...
from app.service.pipeline.tasks.ingest_aemo_wem import _fetch_historical_range

log = get_logger(__name__)

_TABLE = "aemo_wem_dispatch"
...
@click.option(
    "--output",
    default="data/historical_wem.duckdb",
    show_default=True,
...
def main(date_from: str, date_to: str, output: str) -> None:
    ...
    log.info("backfill_wem_to_duckdb.starting", date_from=str(d_from), date_to=str(d_to))
    df = asyncio.run(_fetch_historical_range(start, end))

    if df.empty:
        log.warning("backfill_wem_to_duckdb.no_rows_fetched")
