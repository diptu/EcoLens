#!/usr/bin/env bash
# Applies every services/waerehouse/app/migrations/*.sql file, in order,
...
# this script itself does no bookkeeping about what already ran. Same
# shape as services/data-pipeline/scripts/apply-migrations.sh.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SERVICE_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
MIGRATIONS_DIR="$SERVICE_DIR/app/migrations"
